...
- 2026-07-21: Independent final review put the branch on HOLD after reproducing app-launch bytecode writes inside the bundle. `-I` ignored `PYTHONDONTWRITEBYTECODE`; the native launcher now passes explicit `-B`. Closure requires a pristine rebuild plus identical full-tree hashes before/after the actual app lifecycle smoke.
- 2026-07-21: Review also reproduced daemonized PostgreSQL surviving engine `SIGKILL` and blocking restart. Embedded lifecycle now holds an owner-private nonblocking lock, writes an atomic runtime/postmaster identity record before and after startup, reclaims only an exact matching orphan, and refuses unmarked/mismatched clusters. A real bundled-PostgreSQL fork/crash/restart test proves orphan detection, safe stop, restart, pgvector health, and final cleanup.
- 2026-07-21: Independent re-review approved `91e6bff0ae54` with no Critical or Important findings. The exact staged app then passed artifact-SBOM preflight before and after normal launch with identical full bundle hashes (`2555f465d6f75cb20d6b9ca0cacbc3c47942cbd49e56246cd9238b7c94729012`), authenticated readiness, pgvector 0.8.5, 12 migrations, and clean quit. A separate actual bundled-engine `SIGKILL` smoke reclaimed postmaster 6403, restarted as 6437, served pgvector, and left no final process.
- 2026-07-21: Full regression passed at code commit `d303660`: 1313 Python tests passed, one optional ONNX test skipped, 55 Swift tests passed, Swift product build passed, and lock/Ruff/format/strict-mypy/parser-governance gates passed.
