...
| 2026-07-19 | Phase 2D indexes + atomic publication (dev) | Added retrieval publication contracts (`INDEX_VALIDATED`/`PUBLISHED`), fixture-only deterministic embeddings (production embedding models blocked), migration `0008` staging dense pgvector + FTS lexical + exact-identifier rows, atomic activate/rollback active pointer, and loopback index/publish APIs. Staging remains non-searchable until activation. Docling untouched. | **989** Python and **44** Swift tests; ruff/mypy/lock; indexing contract/embedder/staging/activation/API tests. Tip through `88df534`. | Phase 3 hybrid retrieval; production embedding promotion; criterion 15; OCR/parser promotion; interrupt/resume; Tantivy optional after benchmark. |
