...
| 25–40 | Retrieval | Not proven | Hybrid dense/lexical/exact, intent-weighted RRF, stdlib lexical post-RRF rerank, coverage/abstention. Offline `oss_token_hash_v1` candidate embedder + fail-closed ONNX artifact path governed; install supports `--download` with pinned MiniLM URL+sha256 (opt-in egress only). Fixture default for tests. No sealed-holdout `PRODUCTION` promotion, neural cross-encoder, expansion, multi-hop, or injection suite. | All required planes, identity/as-of, intent, expansion, fusion/rerank, coverage/contradiction/repair/abstention, injection containment, degradation, replay, and promotion gates. |
...
| Retrieval | Not proven | Hybrid RRF + lexical rerank; fixture + OSS token-hash candidate; ONNX fail-closed without artifact; pinned MiniLM `--download` install; no sealed PRODUCTION promotion. |
...
| Local models | Not proven | Candidate OSS embedders + fail-closed ONNX; MiniLM install pinned; sealed qualification remain. |
