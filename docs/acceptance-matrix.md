...
| --- | --- | --- |
| Immutable evidence | Not proven | Phase 1 proves content-addressed bytes, owner/mode/link/size enforcement, corruption detection/recovery, and restart persistence; raw envelopes, supersession history, and full scrub remain. |
...
| Native application | Not proven | Phase 1 proves live runtime/fault/remediation, appearance, accessibility order, compact layout, keyboard refresh, and multi-window behavior; research surfaces and release proof remain. |
| Zero egress | Not proven | Development `atlas doctor` denies external TCP and mDNS while allowing only the exact PostgreSQL socket; exact signed release-artifact proof remains. |
