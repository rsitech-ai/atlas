...
| --- | --- | --- |
| 2026-07-20 | At exact commit `19a72ec553fc`, `script/codex_full_regression.sh` passed with **1231 passed, 1 skipped**; **51 Swift tests passed**; the Swift product built; lock, Ruff check/format, strict mypy, parser governance, and diff checks passed. New public-runner tests prove stdout overflow is classified as `worker_output_too_large`, timeout terminates the recorded process-group leader, and both failures remove partial output. The overflow test failed against pre-drain commit `8e46baf` with the historical `worker_timeout` misclassification. A fresh `atlas doctor --json` still reported `resource_policy=blocked`, so the foreground development smoke was not run. | Worker-supervision regression evidence is stronger and exact-head repository evidence remains green. Host resource admission is still **blocked:external**; no foreground runtime pass, production model, package, signing, notarization, or clean-install claim is added. Every Section 33 and Appendix D status remains **Not proven**. |
