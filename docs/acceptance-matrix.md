...
| 85–102 | Models, evaluation, and observability | Not proven | Phase 1 now has isolated metadata-only OpenTelemetry, exact safe span/context registries, owner-private cross-process JSONL, raw-SDK defense, and current development-component zero-egress evidence through `01c6693`. No model/evaluation plane, distributed Swift-to-publication trace, retention system, or release-artifact proof exists yet. | Interchangeable qualified providers, Apple fallback, isolated services, load/unload/arbiter/recovery, immutable artifacts, six dataset splits, evaluator ordering/calibration/statistics, regression mining, distributed local traces/privacy/retention and disabled exporters. |
